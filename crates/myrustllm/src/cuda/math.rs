...
impl std::ops::Sub for &CUDATensor<CUDAF32> {
    type Output = CUDATensor<CUDAF32>;
...
impl std::ops::Mul for &CUDATensor<CUDAF32> {
    type Output = CUDATensor<CUDAF32>;
...
impl std::ops::Div for &CUDATensor<CUDAF32> {
    type Output = CUDATensor<CUDAF32>;
...
impl std::ops::Neg for &CUDATensor<CUDAF32> {
    ...
    fn neg(self) -> Self::Output {
        let dims = self.dims();
        let mut out = CUDATensor::from_shape(&self.shape());

        unsafe {
            let self_ptr = self.borrow().as_ptr() as *const libc::c_float;
            let out_ptr = out.borrow_mut().as_mut_ptr() as *mut libc::c_float;

            interface::cuda_tensor_neg_f32(
