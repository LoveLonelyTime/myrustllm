...
use myrustllm::optim::optimizer::Optimizer;
use myrustllm::optim::sgd::SGD;
...
fn main() {
    ...
    let input = TensorGrad::leaf(
        GenericTensor::fill_ones(&Shape::from([64, 512]), DType::F32, Device::CPU),
        true,
    );
    ...
    // let mut optim = SGD::new(&mut m, 0.1, None);
    // optim.step();
